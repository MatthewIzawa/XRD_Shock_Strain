---
title: JAC N=32 expansion — capillary subtraction + 4 new samples + 2 lab→synchrotron upgrades — design
date: 2026-05-15
status: design
...
Produce a v2 JAC manuscript with `N = 32` samples spanning 8 parent-body
...
- Update Table 1 (28 rows → 32 rows; 18+ PDF FWHM values; 2 row replacements),
...
| Decision | Choice |
|---|---|
| Scope of new-data inclusion | Full inclusion (N=32) |
...
1. **Verify pre-expansion inventory against the canonical CSV**
   ...
   17 I11 + 4 Lab Co = 28 (the filename says "29samples" but the CSV
   has 28 rows; "29" was a v1 prose miscount that this spec inherited),
   with the I11 breakdown of 11 HED + 6 non-HED.
...
4. **Re-run the PDF first-peak fit on the existing 17 I11 samples**
...
**Goal.** Every data-bearing figure reflects N=32 with capillary-subtracted
...
counts, and PDF backgrounds is coherent for N=32, capillary subtraction,
...
| Section | Change |
|---|---|
| Abstract | "28-meteorite suite spanning seven parent-body groups" → "32-meteorite suite spanning eight parent-body groups". Capillary subtraction *not* added (per decision). |
...
- **Cover letter.** "28-meteorite suite spanning seven parent-body
  groups" → updated for N=32 and 8 groups. Capillary subtraction
...
| Submission timing pressure (external) | 5 | Negotiate; or fall back to "submit N=28 now, revise in review" with a quick-pivot cover letter. |

## 12. Success criteria

- v2 manuscript correctly cites N=32 and 8 parent-body groups throughout.
